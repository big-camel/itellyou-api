...
public class VerifyController {
    ...
    @PostMapping("/mobile")
    public Result mobile(UserInfoModel userModel , HttpServletRequest request , HttpServletResponse response , @MultiRequestBody @NotBlank String code){
        ...
        if(result != 1) return new Result(500,"写入验证信息错误");

        CookieUtils.setCookie(response,VERIFY_COOKIE_KEY,key,"/", VERIFY_EXPIRED);

        return new Result(DateUtils.getTimestamp() + VERIFY_EXPIRED);
    }

    @PostMapping("/email")
    public Result email(UserInfoModel userModel , HttpServletRequest request , HttpServletResponse response , @MultiRequestBody @NotBlank String code){
        if(userModel == null) return new Result(403,"未登陆");
