...
public class QuestionDocController {
    ...
    @PostMapping("/create")
    public Result create(HttpServletRequest request, UserInfoModel userInfoModel, @MultiRequestBody(required = false) String title, @MultiRequestBody String content, @MultiRequestBody String html, @MultiRequestBody String save_type){
    ...
    public Result draft(HttpServletRequest request, UserInfoModel userInfoModel, @PathVariable Long id, @RequestParam(required = false)  boolean ot){
        ...
        return new Result(questionDetailModel,new Labels.LabelModel(QuestionDetailModel.class,"draft","collab"));
    }
    ...
    public Result content(HttpServletRequest request,UserInfoModel userInfoModel,@PathVariable Long id,@MultiRequestBody(required = false) String title,@MultiRequestBody String content,@MultiRequestBody String html,@MultiRequestBody String save_type){
        ...
        try{
            ...
            return new Result(detailModel,new Labels.LabelModel(QuestionDetailModel.class,"draft"));
        }catch (Exception e){
    ...
    public Result publish(HttpServletRequest request, UserInfoModel userInfoModel, @PathVariable Long id, @MultiRequestBody @NotNull Long[] tags, @MultiRequestBody @NotNull Map<String,Object> reward, @MultiRequestBody(required = false) String remark){
        if(userInfoModel == null){
            return new Result(401,"未登录");
        }
        ...
        if(rewardType == RewardType.DEFAULT){
            ...
            if(rewardValue < config.getMin()){
                return new Result(0,"积分悬赏最低" + rewardValue + config.getUnit());
            }
            if(rewardValue > config.getMax()){
                return new Result(0,"积分悬赏最高不超过" + rewardValue + config.getUnit());
            }
            if(bankModel != null && bankModel.getCredit() < rewardValue){
                return new Result(1001,"积分余额不足",bankModel);
            }
        }else if(rewardType == RewardType.CASH){
            RewardConfigModel config = rewardConfig.get(RewardType.CASH);
            if(rewardValue < config.getMin()){
                return new Result(0,"现金悬赏最低" + rewardValue + config.getUnit());
            }
            if(rewardValue > config.getMax()){
                return new Result(0,"现金悬赏最高不超过" + rewardValue + config.getUnit());
            }
            if(bankModel != null && bankModel.getCash() < rewardValue){
                return new Result(1002,"用户余额不足",bankModel);
            }
        }
        ...
        try {
            ...
            if(versionModel == null) return new Result(0,"发布提问失败");
