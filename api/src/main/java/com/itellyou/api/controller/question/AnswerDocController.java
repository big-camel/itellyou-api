...
public class AnswerDocController {
    ...
    @PostMapping("/create")
    public Result create(HttpServletRequest request, UserInfoModel userInfoModel,@PathVariable Long questionId,@MultiRequestBody String content, @MultiRequestBody String html, @MultiRequestBody String save_type){
        ...
        if(answerModel != null){
            try{
                ...
                if(answerModel.isPublished()){
                    return new Result(0,"只能创建一个回答");
                }
                if(answerModel.isDisabled() || answerModel.isDeleted()) return new Result(0,"回答状态不正确");
                ...
                if(versionModel == null) return new Result(0,"创建失败");
                return new Result(answerModel.getId());
            }catch (Exception e){
    ...
    public Result draft(HttpServletRequest request, UserInfoModel userInfoModel,@PathVariable Long id,@PathVariable Long questionId, @RequestParam(required = false) boolean ot){
        if(userInfoModel == null){
            return new Result(401,"未登录");
        }
        //获取回答详情
        ...
        return new Result(detailModel,new Labels.LabelModel(QuestionAnswerDetailModel.class,"draft","collab"));
    }

    @PutMapping("/{id:\\d+}/content")
    public Result content(HttpServletRequest request,UserInfoModel userInfoModel,@PathVariable Long id,@PathVariable Long questionId,@MultiRequestBody String content,@MultiRequestBody String html,@MultiRequestBody String save_type){
        if(userInfoModel == null){
            return new Result(401,"未登录");
        }
        String clientIp = IPUtils.getClientIp(request);
        ...
        try {
            ...
            if(answerModel == null || answerModel.isDisabled() || answerModel.isDeleted()) return new Result(404,"无可用回答");
            //暂时只能创建者有权限编辑
            ...
            return new Result(detailModel,new Labels.LabelModel(QuestionAnswerDetailModel.class,"draft"));
        }catch (Exception e){
            return new Result(0,e.getMessage());
        }
    }
    ...
    public Result rollback(HttpServletRequest request,UserInfoModel userInfoModel,@PathVariable(required = false) Long id,@PathVariable Long questionId,@MultiRequestBody Long version_id){
        if(userInfoModel == null){
            return new Result(401,"未登录");
        }
        String clientIp = IPUtils.getClientIp(request);
        ...
        if(id == null){
            ...
            if(questionModel == null) return new Result(404,"question is not find ");
            //一个用户只能创建一个正常状态的回答
    ...
    public Result publish(HttpServletRequest request, UserInfoModel userInfoModel, @PathVariable Long questionId, @PathVariable Long id, @MultiRequestBody String remark){
        if(userInfoModel == null){
            return new Result(401,"未登录");
        }
        QuestionAnswerDetailModel detailModel = searchService.getDetail(id,questionId,"draft",null,null,true,null,false,null,false);
    ...
    public Result collab(@MultiRequestBody @NotBlank String token, @PathVariable Long questionId, @PathVariable Long id){
