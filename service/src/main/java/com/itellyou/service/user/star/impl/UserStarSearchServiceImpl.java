...
public class UserStarSearchServiceImpl implements UserStarSearchService {
    ...
    public int count(Long userId, Long followerId, Long beginTime, Long endTime, Long ip) {
        return starDao.count(userId != null ? new HashSet<Long>(){{ add(userId);}} : null,followerId, beginTime,endTime,ip);
    }
    ...
    @Override
    @Cacheable(key = "T(String).valueOf(#userId).concat('-').concat(#followerId).concat('-detail')",unless = "#result == null")
